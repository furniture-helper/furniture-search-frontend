...
export default async function Product() {
    const productDataResponse = await fetch(`${process.env.API_BASE_URL}/products/random`, {
        cache: "no-store",
    });
    const productData: ProductDetails = await productDataResponse.json();

    const priceHistoryResponse = await fetch(`${process.env.API_BASE_URL}/products/price-history?url=${productData?.url}`, {
        cache: "no-store",
    });
